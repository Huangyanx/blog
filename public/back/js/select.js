...
(function ($) {
    ...
    mySelect.prototype={
        ...
        initOption: function () {
            //初始化输入框和option
            this.ele.find(".inputWrap").empty();
            this.ele.find(".mySelect-option").empty();//清空之前的
        ...
        refreshOption:function (data,muli) {
            this.ele.find(".mySelect-option").empty();
                var len=data.length;
                for(var i=0;i<len;i++){
                    this.ele.find(".mySelect-option").append('<div data-value="'+data[i]['value']+'"><span>'+data[i]['label']+'</span><i class="fa fa-check"></i></div>');
                }
            this.options=$.extend({},muli,data);
            this.pubFunction();
            this.addEvent();
            //this.refreshInput();
        }
