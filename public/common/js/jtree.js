...
(function($){
 	$.extend($.fn, {
			...
			return this.each(function(){
				...
				var temptree = setTimeout(function(){
					...
					$("a", $this).click(function(event){
						...
						//if (!$(this).attr("target")) return false;  禁止a 标签的链接跳转
					});
                    $this.open_item(op.open_item);
				},1);
			});
		},
		...
		genTree:function(options) {
			...
			return this.each(function(){
				...
				if (tree.size()>0) {
					...
					tree.slideToggle();
					$(">div>div", node).click(function(){//$(">div>div:first  折叠菜单  删掉了>div>a
		...
		open_item:function (obj) {
			//另外添加 进行展开对应节点
                $(obj).parents('ul').prev().find('.folder_expandable').trigger('click');
        }
	});
})(jQuery);